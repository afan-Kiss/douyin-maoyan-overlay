...
const { launchLoginBrowser, safeKillProcess, safeKillPid, killProcessesOnPort } = require("./chrome-launcher");
...
async function openLoginPage(launched, loginUrl) {
  ...
  return {
    ...
    debugPort: launched.debugPort || 0,
    browserPids: Array.isArray(launched.browserPids) ? launched.browserPids : [],
  };
}

async function closeLoginBrowser(session) {
  if (!session) return;
  // Windows CDP：browser.close 可能挂起；stub 退出后必须按 PID/端口强杀真正 Chrome
  const closeWithTimeout = async (fn) => {
    try {
      await Promise.race([
  ...
  for (const pid of session.browserPids || []) {
    safeKillPid(pid);
  }
  if (session.child) {
  ...
  if (session.debugPort) {
    killProcessesOnPort(session.debugPort);
  }
}
...
async function runMaoyanLogin(dataDir, options = {}) {
  ...
  const scheduleBrowserCleanup = () => {
    if (!session) return;
    void closeLoginBrowser(session).catch(() => {});
  };

  const emitAttemptResult = (payload) => {
    if (!payload || currentLoginAttemptId !== attemptId) return;
    emitLoginResult({ ...payload, attemptId });
  };
    ...
    (async () => {
      ...
      try {
        const result = await waitForBrowserLoginAndSave(session.context, session.browser, storageState, {
          ...
          browserPids: session.browserPids || [],
        });
        ...
        if (result.ok && result.saved && (detailReady || (result.loginCookieReady && identityOk))) {
          console.log("猫眼登录信息已保存", storageState);
          try {
            ...
            const loginPatch = {
              ...
              signatureCaptured: detailReady,
              detailPayloadValid: detailReady,
              accountLoggedIn: true,
              sessionUsable: detailReady || identityOk,
              loginRequired: false,
              lastVerifyError: detailReady ? null : result.lastVerifyError || "mtgsig_deferred",
              lastVerifyAt: new Date().toISOString(),
            };
            mergeSessionStatus(loginPatch, dataDir);
            saveSessionCapability(dataDir, {
              capability: loginPatch,
              lastDetailApiSuccessAt: detailReady ? loginPatch.lastVerifyAt : null,
              savedAt: loginPatch.lastVerifyAt,
            });
          } catch (error) {
        ...
        if (result.ok && result.loginCookieReady && !identityOk) {
          cleanupInvalidLoginState(storageState);
          failurePayload = {
            ok: false,
      ...
      restoreAlwaysOnTop(restoreOnTopWindow);
      scheduleBrowserCleanup();
      if (failurePayload) emitAttemptResult(failurePayload);
    })();
