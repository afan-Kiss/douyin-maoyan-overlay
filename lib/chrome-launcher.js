...
function safeKillPid(pid) {
  ...
  try {
    if (process.platform === "win32") {
      spawn("taskkill", ["/PID", String(n), "/T", "/F"], {
        stdio: "ignore",
        windowsHide: true,
      });
      return;
...
/** 查找监听指定本地端口的 PID（用于 Chrome stub 退出后杀掉真正浏览器进程） */
...
async function launchViaCdp(chromePath, profileDir, startUrl) {
  ...
  try {
    ...
    // stub 可能已退出：记下真正占用调试端口的 Chrome PID，关闭时用来强杀
    const browserPids = findPidsListeningOnPort(port).filter((pid) => pid !== child.pid);
    return {
      mode: "cdp",
      browser,
      context,
