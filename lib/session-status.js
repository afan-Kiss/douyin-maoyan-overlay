...
function applySessionApiError(code) {
  ...
  if (["login_required", "upstream_401", "401", "detail_http_401"].includes(normalized)) {
  ...
  return fetchJson(url, 5000)
    .then((serverStatus) => {
      ...
      if (serverStatus.detailApiReady === false && cachedStatus.detailApiReady === true) {
        patch.detailApiReady = false;
        patch.browserSessionVerified = false;
        patch.signatureReady = serverStatus.signatureReady === true ? cachedStatus.signatureReady : false;
        patch.accountLoggedIn = false;
      }
