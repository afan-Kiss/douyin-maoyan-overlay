...
const hydratedDirs = new Set();
let lastSessionDataDir = "";
let startupVerifyDone = false;

function resolveSessionDataDir() {
  if (lastSessionDataDir) return lastSessionDataDir;
  if (process.env.MAOYAN_DATA_DIR) return process.env.MAOYAN_DATA_DIR;
  try {
...
function deriveSessionFields(partial = {}) {
  ...
  const sigReady = partial.signatureReady ?? cachedStatus.signatureReady;
  const loginRequired = Boolean(partial.loginRequired ?? cachedStatus.loginRequired);
  const sessionUsable =
    partial.sessionUsable ??
    Boolean((identity && detail && !loginRequired) || (detail && sigReady && !loginRequired));
  return {
    identityCookieExists: Boolean(identity),
    loginCookieReady: Boolean(identity || (detail && sigReady)),
    accountLoggedIn: Boolean(identity || (detail && sigReady && !loginRequired)),
    productionDetailReady: Boolean(detail),
    loginRequired,
    sessionUsable: Boolean(sessionUsable),
  };
}

function persistLocalSessionStatus(dataDir) {
  if (!dataDir) return;
  const status = getSessionStatus(dataDir);
  if (status.loginRequired) {
    clearSessionCapability(dataDir);
    return;
  }
...
function hydrateSessionFromDisk(dataDir) {
  if (!dataDir || hydratedDirs.has(dataDir)) return;
  hydratedDirs.add(dataDir);

  const sync = readSyncStatus(dataDir);
  const snapshot = loadSessionCapability(dataDir);
  if (!snapshot || !fingerprintMatches(dataDir, snapshot)) {
    mergeSessionStatus({ ...sync });
    return;
  }
  ...
  if (
    ...
    Object.assign(patch, cap);
    patch.loginRequired = false;
    patch.sessionUsable = Boolean(
      sync.identityCookieExists && cap.detailApiReady,
    );
  } else if (isPersistedSessionUsable(dataDir)) {
    Object.assign(patch, cap);
  }

  const cacheDir = path.join(dataDir, "session_cache");
  const sigAt = newestSignatureTimestamp(cacheDir);
  if (sigAt && !patch.lastVerifyAt) {
    patch.lastVerifyAt = sigAt;
  }
  if (snapshot.savedAt) {
    patch.lastVerifyAt = snapshot.savedAt;
  }

  mergeSessionStatus(patch);
}

function mergeSessionStatus(partial = {}, dataDir = null) {
  ...
  if (dataDir) persistLocalSessionStatus(dataDir);
  return getSessionStatus(dataDir);
}

function getSessionStatus(dataDir) {
  if (dataDir) {
    lastSessionDataDir = dataDir;
    hydrateSessionFromDisk(dataDir);
  }
...
function shouldScheduleVerify(apiBase, options = {}) {
  ...
  if (verifyRunning) {
    if (options.force || options.startup) forceRerunRequested = true;
    return false;
  }
  if (options.force) return true;
  // 进程启动后至少验签一次，避免 hydrate 的 5 分钟 TTL 跳过后台校验
  if (options.startup && !startupVerifyDone) {
    startupVerifyDone = true;
    return true;
  }
...
function applySessionApiError(code) {
  ...
  const dataDir = resolveSessionDataDir();
  ...
  if (
    ...
    if (dataDir) clearSessionCapability(dataDir);
    return mergeSessionStatus(patch, dataDir);
  }
  ...
  return getSessionStatus(dataDir || undefined);
}
