...
async function verifyDetailOnBoxPage(page, movieId, boxLevel = "1", options = {}) {
  ...
  const triggerSignedFetches = async (id) => {
    ...
    const urls = [
      ...
      // 登录阶段就把预测/下映等签名抓齐，避免登录后还要无头补签拖慢明细
      `/i/api/movie/getPredictionBox?movieId=${mid}&WuKongReady=h5`,
      `/i/api/movie/getTechData?movieId=${mid}&WuKongReady=h5`,
      `/i/api/movie/getBoxShowna?movieId=${mid}&WuKongReady=h5`,
      `/i/api/movie/getWantData?movieId=${mid}&WuKongReady=h5&token=`,
