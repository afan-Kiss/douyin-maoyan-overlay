...
function buildDetailApiPath(movieId, boxLevel = "1") {
  return `/i/api/movie/getBoxShow?movieId=${movieId}&boxLevel=${boxLevel}&yodaReady=h5&csecplatform=4&csecversion=4.3.0`;
}
...
async function verifyDetailOnBoxPage(page, movieId, boxLevel = "1") {
  ...
  const rememberRequest = (req) => {
    ...
    const url = req.url();
    if (!url.includes("getBoxShow") || !url.includes(targetSub)) return;
  ...
  try {
    ...
    if (!captured.headers?.mtgsig) {
      const apiPath = buildDetailApiPath(movieId, boxLevel);
    ...
    if (!signatureCaptured) lastVerifyError = "sig_capture_failed";
    else if (!httpOk) {
      lastVerifyError =
        detailHttpStatus === 401 ? "login_required" : detailHttpStatus === 403 ? "upstream_403" : `http_${detailHttpStatus}`;
...
async function verifyCapabilitiesInContext(context, options = {}) {
  ...
  try {
    ...
    const accountLoggedIn = Boolean(identityCookieExists && detailApiReady);
