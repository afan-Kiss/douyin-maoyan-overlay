...
const { verifyCapabilitiesInContext, isMaoyanLoginRedirect } = require("./session-capability");
  ...
  try {
    ...
    return {
      ...
      loginRequired:
        ...
        result.lastVerifyError === "detail_http_401" ||
        result.lastVerifyError === "login_required",
...
async function runHeadlessHeadedDiagnostic(storageStatePath, options = {}) {
  ...
  for (const { mode, headless } of modes) {
    ...
    try {
      ...
      if (isMaoyanLoginRedirect(captured.finalUrlHost, captured.finalPath)) {
        captured.lastVerifyError = "login_required";
      } else if (!captured.boxPageLoaded) captured.lastVerifyError = "box_page_not_loaded";
