...
async function streamChunkOnce({
  ...
  if (status === 206) {
    ...
    // 允许服务端多返回尾部，本端按 expectedLen 截断读取即可
    if (respStart !== expectedStart || respEnd < expectedStart) {
      throw new Error(
    ...
    // 多返回时 Content-Length 可能大于本分片；不足才算错
    if (contentLength > 0 && contentLength < expectedBytes) {
      throw new Error(
        `Content-Length mismatch for range: expected at least ${expectedBytes}, got ${contentLength}`,
      );
    }
  }
  ...
  try {
    ...
    while (true) {
      ...
      if (sinceReport >= PROGRESS_EVERY_BYTES) {
        sinceReport = 0;
        // 进度刷新不 fsync：每 64KB 刷盘会把下载拖到极慢；完整性靠分片长度 + 整包 SHA
