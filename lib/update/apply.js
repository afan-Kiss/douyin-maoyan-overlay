...
const HEALTH_WAIT_TIMEOUT_SECS = 90;
const UPDATE_ROLLBACK_BLOCKED_PROCESS_ALIVE = 12;
const UPDATE_ROLLBACK_FAILED = 13;
const FILE_UNLOCK_RETRIES = 30;
const FILE_UNLOCK_DELAY_MS = 200;

const _actionLog = [];
let _testDeps = null;
let _testHealthWaitTimeoutMs = 0;
...
function _resetApplyTestState() {
  _actionLog.length = 0;
  _testDeps = null;
  _testHealthWaitTimeoutMs = 0;
}

function _setHealthWaitTimeoutMs(ms) {
  _testHealthWaitTimeoutMs = Number(ms) || 0;
}
...
async function killProcessWindows(pid, timeoutMs = 10000) {
  ...
  const outcome = await waitForPidExitWindows(pid, timeoutMs);
...
async function waitForHealthConfirmation(token, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    const ack = deps.consumeHealthOk(token);
    if (ack) return true;
...
async function rollbackFailedHealthUpdate({
  ...
  if (newPid) {
    const killed = await deps.killProcess(newPid, 10000);
    if (!killed) {
      applyLog(
        installDir,
        `UPDATE_ROLLBACK_BLOCKED_PROCESS_ALIVE pid=${newPid} reason=${reason}`,
      );
      recordAction("ROLLBACK_BLOCKED_PROCESS_ALIVE");
      return UPDATE_ROLLBACK_BLOCKED_PROCESS_ALIVE;
    }
  }
  const rolled = rollbackFromBackup(target, bak, installDir, "health_timeout", reason);
  if (!rolled) {
    applyLog(
      installDir,
      `UPDATE_ROLLBACK_FAILED targetExe=${target} backupExe=${bak} reason=${reason}`,
    );
    recordAction("ROLLBACK_FAILED");
...
async function runApplyUpdate(args) {
  ...
  try {
    ...
    const rolled = rollbackFromBackup(target, bak, installDir, "restart", error.message);
    if (rolled) {
      deps.clearPendingUpdate();
      deps.writeVersionFile(installDir, previousVersion);
    }
  ...
  const healthy = await waitForHealthConfirmation(
    healthToken,
    _testHealthWaitTimeoutMs || HEALTH_WAIT_TIMEOUT_SECS * 1000,
  );
  if (healthy) {
    ...
    return rollbackFailedHealthUpdate({
      installDir,
      exeName: args.exeName,
      target,
      bak,
...
function scheduleApplyAndExit(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      finish(reject, new Error("更新助手启动超时"));
    }, 8000);
