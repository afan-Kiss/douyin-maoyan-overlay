...
function mapReason(codeOrReason) {
  ...
  if (raw === "captcha" || raw === "google_captcha") return "captcha";
  if (raw === "forbidden" || raw === "403") return "403";
  if (raw === "network" || raw === "network_error") return "network_error";
  ...
  if (raw === "download" || raw === "download_fail" || raw === "download_failed") return "download_failed";
  if (raw === "cache_error") return "cache_error";
  if (raw === "query-mismatch" || raw === "banned-url") return raw;
  return raw || "network_error";
}
...
function logPosterSearch(fields) {
  ...
  for (const [key, value] of Object.entries(fields)) {
    ...
    // 禁止输出 cookie / token / 用户信息
    if (/cookie|authorization|set-cookie|token|password|secret|userinfo|user-info/i.test(key)) continue;
...
async function searchOfficialPoster(movieName, options = {}) {
  ...
  try {
    ...
    logPosterSearch({ ...diagBase, result: "fail", reason: "network_error" });
  ...
  try {
    ...
    if (blocked) {
      const reason = status === 403 ? "403" : "captcha";
      logPosterSearch({ ...diagBase, cacheHit: false, reason });
