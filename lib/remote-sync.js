...
function startRemoteSync(remoteAdminUrl, onChange, options = {}) {
  stopRemoteSync();
  const base = String(remoteAdminUrl || "").replace(/\/$/, "");
  if (!base) return;
  ...
  const intervalMs = Number(options.intervalMs) > 0 ? Number(options.intervalMs) : 15000;
  ...
  sync();
  remoteSyncTimer = setInterval(sync, intervalMs);
}
