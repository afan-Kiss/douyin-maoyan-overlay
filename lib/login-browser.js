...
function persistLoginCapturedSignature(dataDir, capabilities = {}) {
  ...
  // 预测/下映等扩展接口签名（缺了就会字段空、登录后很慢）
  const apiSigs = capabilities.apiSignatures || {};
  let wukongCount = 0;
  for (const [slug, captured] of Object.entries(apiSigs)) {
    const name = String(slug || "").replace(/[^\w-]/g, "");
    if (!name || name === "getBoxShow") continue;
    const sigHeaders = captured?.headers;
    if (!sigHeaders?.mtgsig) continue;
    const apiPath = `/i/api/movie/${name}`;
    const filePath = path.join(cacheDir, `wukong_${movieId}_${name}.json`);
    const payload = {
      movieId,
      apiPath,
      url: captured.url || "",
  ...
  if (wrote) {
    report(
      "info",
      "login",
      `login_sig_cached movieId=${movieId} source=${capabilities.signatureSource || "login_browser"} wukong=${wukongCount}`,
    );
  }
  return wrote;
}
    ...
    const { result, reused } = await runLoginVerifySingleflight(async () => {
      ...
      if (detail.detailApiReady) {
        ...
        const capabilities = {
          ...
          apiSignatures: detail.apiSignatures || {},
