...
async function checkAndDownloadUpdate(updateManager, splash) {
  ...
  if (!isPackagedApp() || skipAutoUpdate() || !updateManager.updateCheckConfigured()) {
    safeSend({ phase: "loading", message: "正在进入主程序…", percent: 90, showBar: true });
    return false;
  }
  ...
  try {
    safeSend({ phase: "checking", message: "正在检查更新…", percent: 8, showBar: true });
    ...
    // 同版本哈希命中 / 已安装态跳过：不是失败，勿当成告警再卡 700ms
    if (/无需安装|无需重复更新|已是最新/.test(msg)) {
      safeSend({
        phase: "loading",
        message: "当前已是最新版本",
        percent: 90,
